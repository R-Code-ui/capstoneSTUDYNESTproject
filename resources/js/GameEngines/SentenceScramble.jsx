...
function SortableWord({ id, word }) {
    ...
    return (
        <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="px-6 py-3 rounded-2xl bg-indigo-500 text-white font-bold shadow-[0_4px_0_rgb(67,56,202)] cursor-grab active:cursor-grabbing touch-none select-none">
            {word}
        </div>
    );
}
    ...
    return (
        <GameShell title="Sentence Scramble" description={content.description} roundLabel={`Round ${roundIndex + 1} of ${rounds.length}`} onExit={onExit}>
            <div className="flex flex-col items-center gap-8 p-6 bg-white rounded-3xl border border-indigo-100 shadow-xl max-w-2xl mx-auto">
                    ...
                    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                        <SortableContext items={items.map((i) => i.id)} strategy={horizontalListSortingStrategy}>
                            <div className="flex flex-wrap gap-3 justify-center">
                                {items.map((item) => <SortableWord key={item.id} id={item.id} word={item.word} />)}
                            </div>
                        </SortableContext>
                    </DndContext>
                </div>
                ...
                {!feedback && <PrimaryButton type="button" onClick={handleCheck} className="text-lg px-8 py-3">Check My Sentence</PrimaryButton>}
            </div>
        </GameShell>
    );
}
