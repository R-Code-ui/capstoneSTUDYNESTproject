...
export default function NotificationBell() {
    ...
    return <div className="relative">
        ...
        {open && <>
            ...
            <div className="notification-dropdown absolute right-0 z-20 mt-3 w-[min(24rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
                ...
                <div className="max-h-96 overflow-y-auto">
                    {notifications.items.length === 0 ? <div className="px-6 py-10 text-center text-sm text-slate-500">You are all caught up.</div> : notifications.items.map((notification) => <button key={notification.id} type="button" onClick={() => openNotification(notification)} className={`flex w-full min-w-0 gap-3 border-b border-slate-100 px-4 py-3 text-left transition hover:bg-slate-50 ${notification.read_at ? 'bg-white' : 'bg-indigo-50/50'}`}><Icon type={notification.icon} /><span className="min-w-0 flex-1 overflow-hidden"><span className="flex min-w-0 items-center gap-2"><span className="min-w-0 truncate text-sm font-semibold text-slate-800">{notification.title}</span>{!notification.read_at && <span className={`notification-priority-badge shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-bold uppercase ${colors[notification.priority] || colors.normal}`}>{notification.priority}</span>}</span><span className="notification-message mt-0.5 block truncate text-xs leading-5 text-slate-600">{notification.message}</span><span className="mt-1 block text-[11px] text-slate-400">{notification.created_at}</span></span></button>)}
                </div>
            </div>
        </>}
    </div>;
