...
<body>
    ...
    <div class="meta-bar">
        @if($filters['grade_level'])
            <span>🏫 Grade: <strong>{{ $filters['grade_level'] }}</strong></span>
        @endif
        @if($filters['subject'])
            <span>📘 Subject: <strong>{{ $filters['subject'] }}</strong></span>
        @endif
        @if($filters['term'])
            <span>📅 Term: <strong>{{ $filters['term'] }}</strong></span>
        @endif
    </div>
        ...
        <div class="summary-grid">
            @foreach($summary as $key => $value)
                <div class="summary-card">
                    ...
                    @if($key === 'total_students' && isset($summary['total_completed']))
                        <div class="context">out of {{ $summary['total_students'] }} students</div>
                    @elseif($key === 'total_completed' && isset($summary['total_students']))
                        <div class="context">out of {{ $summary['total_students'] }} students</div>
                    @endif
                </div>
            @endforeach
                ...
                @foreach($data as $row)
                    <tr>
                        @foreach($row as $key => $value)
                            <td>
                                @if(is_numeric($value))
                                    {{ round($value) }}@if(in_array($key, ['completion_rate','passing_rate','participation_rate','average_score','percentage','overall_progress']))% @endif
                                @else
                                    {{ $value ?? '—' }}
                                @endif
                                @if($key === 'completed' && isset($row['total_students']))
                                    <span class="out-of-text"> / {{ $row['total_students'] }}</span>
                                @elseif($key === 'incomplete' && isset($row['total_students']))
                                    <span class="out-of-text"> / {{ $row['total_students'] }}</span>
                                @elseif($key === 'submitted' && isset($row['total_assignments']))
                                    <span class="out-of-text"> / {{ $row['total_assignments'] }}</span>
                                @endif
                            </td>
                        @endforeach
                    </tr>
    ...
    <!-- Footer -->
    <div class="footer">
        Generated by StudyNest – Teacher Reports | {{ now()->format('Y') }}
    </div>
